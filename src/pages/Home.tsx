  ...
  return (
    <div className="flex flex-col bg-slate-50 min-h-screen pb-20">
      ...
      <section className="relative overflow-hidden bg-white border-b border-slate-200">
         <div className="absolute inset-0 bg-blue-50/50 hidden lg:block" />
         ...
         <div className="max-w-[1600px] mx-auto px-6 py-20 lg:py-32 relative z-10 flex flex-col lg:w-1/2">
            <div className="flex items-center gap-2 mb-6">
               ...
               <span className="text-sm font-semibold text-blue-800 uppercase tracking-wider">Trusted by 500,000+ Dealers globally</span>
            </div>
            
            <h1 className="text-5xl md:text-7xl font-display font-extrabold tracking-tight text-slate-900 leading-[1.05] mb-6">
               Find Your Next Vehicle <span className="text-blue-700">At Auction</span>
            </h1>
            ...
            <div className="bg-white p-4 rounded-2xl shadow-xl border border-slate-200 max-w-xl flex flex-col sm:flex-row gap-3">
               <div className="flex-1 relative">
                  <Search className="w-5 h-5 text-slate-400 absolute left-4 top-1/2 -translate-y-1/2" />
                  <input type="text" placeholder="Search Make, Model, or VIN..." className="w-full bg-slate-50 pl-12 pr-4 py-3.5 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 focus:outline-none transition-all font-medium" />
               </div>
            ...
            <div className="mt-10 flex gap-8 whitespace-nowrap overflow-x-auto no-scrollbar pb-4 items-center">
               <div className="flex flex-col">
                 ...
                 <span className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Vehicles Sold</span>
               </div>
               <div className="w-px h-10 bg-slate-200" />
               <div className="flex flex-col">
                 <span className="text-3xl font-bold text-slate-900">100%</span>
                 <span className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Buyer Protection</span>
               </div>
               <div className="w-px h-10 bg-slate-200" />
               <div className="flex flex-col">
                 <span className="text-3xl font-bold text-slate-900">24/7</span>
                 <span className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Live Bidding</span>
               </div>
            </div>
         </div>
      </section>
         ...
         <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
            <div>
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-semibold text-orange-600 uppercase tracking-wider">Ending Soon</span>
               </div>
               <h2 className="text-3xl md:text-4xl font-display font-bold text-slate-900">Live Auctions</h2>
            </div>
            <Link to="/inventory" className="flex items-center gap-2 text-blue-700 font-semibold hover:text-blue-800 transition-colors">
      ...
      {/* Categories */}
      <section className="py-20 max-w-[1600px] mx-auto px-6 w-full">
         <h2 className="text-3xl md:text-4xl font-display font-bold text-slate-900 mb-10">Browse Categories</h2>
         
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {['Exotics & Luxury', 'SUVs & Crossovers', 'Trucks & Commercial', 'Clean Title', 'Salvage Title', 'Classics'].map((cat, i) => (
               <Link to={"/inventory"} key={i} className="enterprise-card flex flex-col items-center justify-center py-10 px-4 hover:-translate-y-1 hover:shadow-lg transition-all text-center group cursor-pointer border-slate-200">
  ...
  return (
    <div className="enterprise-card flex flex-col group relative">
       ...
       <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
          ...
          <div className="absolute top-3 left-3 bg-white/95 backdrop-blur shadow-sm border border-slate-200 px-3 py-1 rounded-md text-[11px] font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
       ...
       <div className="p-5 flex-1 flex flex-col">
          <Link to={`/lot/${lot.id}`} className="block mb-4 hover:text-blue-700 transition-colors">
             <h3 className="font-bold text-lg leading-tight line-clamp-2 text-slate-900">{lot.name}</h3>
             <p className="text-xs text-slate-500 mt-1 uppercase font-semibold">VIN: {lot.id.padEnd(17, '0').substring(0, 17).toUpperCase()}</p>
          </Link>
          ...
          <div className="mt-auto space-y-4">
             ...
             <div className="flex justify-between items-end border-t border-slate-100 pt-4">
                <div>
                   <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Current Bid</span>
                   <span className="text-2xl font-display font-bold text-blue-700">{formatCurrency(lot.currentBid)}</span>
                </div>
                <div className="text-right">
                   <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Time Left</span>
                   <div className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold", isAlert ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-700")}>
             ...
             {/* Action */}
             <Link to={`/lot/${lot.id}`} className="block w-full py-2.5 text-center bg-slate-900 text-white font-semibold rounded-lg hover:bg-slate-800 transition-colors">
                View & Bid
             </Link>
          </div>
       </div>
